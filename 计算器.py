...
def hans2():
    ...
    Label(root2, text = '参数名称').grid(row = 0, column = 0)
    Label(root2, text = '值').grid(row = 0, column = 1)
    Label(root2, text = '表达式（左）').grid(row = 1, column = 0)
    Label(root2, text = '表达式（右）').grid(row = 2, column = 0)
    Label(root2, text = '未知数').grid(row = 3, column = 0)
    ...
    v1 = StringVar()
    e4 = Entry(root2, width=10, textvariable=v1, state='readonly').grid(row=4, column=1)

    def ans():
        x=Symbol(str(e3.get()))
        v1.set(solve(eval(e1.get()) - eval(e2.get()), x))

    Button(root2, text='求解', width=10, command=ans).grid(row=4, column=0, sticky=W, padx=10, pady=5)
...
def hans6():
    ...
    v1 = StringVar()
    e21 = Entry(root6, width=20, textvariable=v1, state='readonly').grid(row=20, column=1)

    v2 = StringVar()
    e22 = Entry(root6, width=20, textvariable=v2, state='readonly').grid(row=21, column=1)

    v3 = StringVar()
    e23 = Entry(root6, width=20, textvariable=v3, state='readonly').grid(row=22, column=1)
...
Button6 = Button(root1, text='游戏攻略', width=10, command = hans6).grid(row=5, column=0, sticky=W, padx=10, pady=5)
Button7 = Button(root1, text='抽卡模拟', width=10, command = hans7).grid(row=6, column=0, sticky=W, padx=10, pady=5)
