...
def hans2():
    ...
    def hans22():
        ...
        def ans():
            x,y = sympy.symbols(str(e5.get()) + str(e6.get()))
            v1.set(sympy.solve([eval(e1.get()) - eval(e2.get()),eval(e3.get()) - eval(e4.get())], [x, y]))
    ...
    Button21 = Button(root2, text='求一元一次方程', width=10, command = hans21).grid(row=0, column=0, sticky=W, padx=10, pady=5)
    Button22 = Button(root2, text='求二元一次方程', width=10, command = hans22).grid(row=1, column=0, sticky=W, padx=10, pady=5)
    Button23 = Button(root2, text='求一元二次方程', width=10, command = hans23).grid(row=2, column=0, sticky=W, padx=10, pady=5)
